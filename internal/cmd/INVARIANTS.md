...
- [tested] ENC-01: `encrypt` is registered as a root subcommand and `rtbtr encrypt --help` succeeds and mentions `encrypt`.
- [tested] ENC-02: `encrypt` produces a valid JSON envelope with `ciphertext` (standard base64), `ephemeral_public_key` (URL-safe base64, 32 bytes), and `algorithm` (`x25519-aes256gcm`) fields.
- [tested] ENC-03: `encrypt` reads plaintext from stdin when `--message` is absent and stdin is not a terminal.
- [tested] ENC-04: `encrypt` rejects empty message content.
- [tested] ENC-05: `encrypt` rejects an invalid (malformed or wrong-length) Ed25519 public key passed via `--to`.
- [tested] ENC-06: `encrypt` rejects input larger than 1,048,576 bytes (1MB) with `message too large (max 1MB)`.
- [tested] ENC-07: `encrypt` requires `--to` flag; rejects when it is missing.
- [tested] ENC-08: `encrypt` is fully offline — does not require a `.rtbtr` home directory or private key.
- [tested] ENC-09: `encrypt` is non-deterministic — encrypting the same plaintext with the same key produces different ciphertext each time.
- [tested] ENC-10: `encrypt` rejects when stdin is a terminal and `--message` is absent.
- [tested] ENC-11: `encrypt` accepts a message of exactly 1MB (boundary case).
- [tested] ENC-12: `--message` flag takes priority over stdin content.
- [tested] ENC-13: `encrypt` rejects whitespace-only message content.
- [tested] ENC-14: `encrypt` rejects oversized input from stdin (>1MB).
- [tested] ENC-15: `encrypt` does not accept positional arguments (`cobra.NoArgs`).

## decrypt

- [tested] DEC-01: `decrypt` is registered as a root subcommand and `rtbtr decrypt --help` succeeds and mentions `decrypt`.
- [tested] DEC-02: `decrypt` roundtrip — encrypting then decrypting recovers the original plaintext, including binary-like content.
- [tested] DEC-03: `decrypt` reads payload from stdin when `--payload` is absent and stdin is not a terminal.
- [tested] DEC-04: `decrypt` rejects invalid JSON payload, payload missing `ciphertext` field, and payload missing `ephemeral_public_key` field.
- [tested] DEC-05: `decrypt` requires a private key — errors with `private key not found` when the `private_key` file is absent from `.rtbtr`.
- [tested] DEC-06: `decrypt` with wrong private key returns an AEAD authentication error (cannot decrypt).
- [tested] DEC-07: `decrypt` rejects when `.rtbtr` directory is missing, with an error mentioning `.rtbtr`.
- [tested] DEC-08: `decrypt` outputs raw plaintext on stdout (no JSON, no metadata headers) for clean piping.
- [tested] DEC-09: `decrypt` roundtrip works with large payloads near the 1MB boundary.
- [tested] DEC-10: `decrypt` rejects payload with tampered ciphertext (AES-GCM authentication failure).
- [tested] DEC-11: `decrypt` rejects empty payload string.
- [tested] DEC-12: `decrypt` does not accept positional arguments (`cobra.NoArgs`).
- [tested] DEC-13: `decrypt` roundtrip works with minimal (single-space) plaintext content.
